...
class OnBoard
    ...
    @fromString: (str, @base) ->
        ...
        whites = []
        lines = str.replace(/(\r?\n)*$/, '').split /\r?\n/
        throw 'bad format' if lines.length isnt @base.BOARD_SIZE

        for line, y in lines
            throw 'bad format' if line.length isnt @base.BOARD_SIZE
    ...
    constructor: (@base, blacks, whites) ->
        ...
        @onBoard = new Array @base.BOARD_SIZE
        for i in [0...@onBoard.length]
            @onBoard[i] = new Array @base.BOARD_SIZE
            for j in [0...@onBoard[i].length]
                @onBoard[i][j] = @base.EMPTY
        for e in blacks
            @onBoard[e[0]][e[1]] = @base.BLACK
        for e in whites
            @onBoard[e[0]][e[1]] = @base.WHITE
    ...
    isEmptyAt: (position) ->
        ...
        switch @stateAt position
            when @base.BLACK, @base.WHITE then false
            else true
    ...
    isLegal: ->
        ...
        for x in [0...@base.BOARD_SIZE]
            for y in [0...@base.BOARD_SIZE] when not @isEmptyAt [x, y]
    ...
    deployment: ->
        ...
        for x in [0...@base.BOARD_SIZE]
            for y in [0...@base.BOARD_SIZE]
                position = [x, y]
                switch @stateAt(position)
                    when @base.BLACK then blacks.push position
                    when @base.WHITE then whites.push position
        [blacks, whites]
    ...
    score: ->
        ...
        ###
        @numOf(@base.BLACK) - @numOf(@base.WHITE)
    ...
    stringAndLibertyAt: (position) ->
        ...
        aux = (unchecked, string, liberty) =>
            ...
            adjacencies = @base.adjacenciesAt checking
    ...
    numOfLiberties: (stone) ->
        ...
        strings = @strings()[switch stone
            when @base.BLACK then 0
            when @base.WHITE then 1]
        for s in strings
    ...
    whoseEyeAt: (position, checkings = []) ->
        ...
        adjacencies = @base.adjacenciesAt position
        return null unless adjacencies.every((e) => @stateAt(e) is @base.BLACK) or adjacencies.every((e) => @stateAt(e) is @base.WHITE)
        ...
        for x in [0...@base.BOARD_SIZE]
            for y in [0...@base.BOARD_SIZE]
                switch @whoseEyeAt [x, y]
                    when @base.BLACK then result[0].push [x, y]
                    when @base.WHITE then result[1].push [x, y]
        result
    ...
    captureBy: (position) ->
        ...
        capturedStone = @base.opponentOf @stateAt position
        adjacencies = @base.adjacenciesAt position
        captives = []
