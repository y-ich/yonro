...
(function() {
  var BitBoardBase, BoardBase, DEBUG, EvaluationResult, OnBoard, boardsToString, cache, check, compare, countBits, e, evalUntilDepth, evaluate, k, l, len, len1, len2, m, onlySuicide, ref, ref1, ref2, ref3, root,
  ...
  self.onmessage = function(event) {
    var error, history, ref4, ref5, result;
    try {
      history = event.data.history.map(function(e) {
        return OnBoard.fromString(e);
      });
      if ((ref4 = history[history.length - 2]) != null ? ref4.isEqualTo(history[history.length - 1]) : void 0) {
        history[history.length - 2] = history[history.length - 1];
        if ((ref5 = history[history.length - 3]) != null ? ref5.isEqualTo(history[history.length - 1]) : void 0) {
